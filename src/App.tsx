...
// Lazy-load heavy map view so it does not bloat the main bundle
const CoverageMap = lazy(() =>
  import('./components/region/CoverageMap').then((m) => ({ default: m.CoverageMap }))
);
  ...
  return (
    <div className="flex min-h-screen bg-mma-light-bg">
      ...
      <main className="min-h-screen flex-1 transition-[margin] duration-300">
        ...
        {activeView === 'coverage-map' && (
          <Suspense fallback={<div className="flex min-h-screen items-center justify-center text-sm text-mma-blue-gray">Loading map…</div>}>
            <CoverageMap />
          </Suspense>
        )}
