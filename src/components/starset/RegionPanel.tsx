...
const NOTION_DB_URL = 'https://www.notion.so/34f750fa613d813d8bf7c3578c5f2cfb';
...
function parseOpportunities(raw: string): Opportunity[] | null {
  if (!raw || !raw.startsWith('__json__')) return null;
  try {
    return JSON.parse(raw.slice(8)) as Opportunity[];
  } catch {
    return null;
  }
}
...
export function RegionPanel({ region, data, onClose, onSave }: RegionPanelProps) {
  ...
  const opportunities = parseOpportunities(data?.areas_of_opportunity ?? '');
    ...
    <div className="flex h-full flex-col bg-white shadow-xl" style={{ width: 380, minWidth: 380 }}>
      ...
      <div className="flex items-center justify-between px-5 py-4" style={{ background: region.color }}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-full bg-white/20 text-white hover:bg-white/30 transition-colors" aria-label="Close panel">
            <X className="h-4 w-4" />
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
        {saved && (
          <div className="rounded-lg bg-green-50 border border-green-200 px-4 py-2.5 text-sm font-medium text-green-700">Saved successfully</div>
        )}

        <Section label="Current Networks of Interest" value={data?.networks_of_interest ?? ''} editValue={editState.networks_of_interest} editing={editing} onChange={(v) => setEditState((s) => ({ ...s, networks_of_interest: v }))} placeholder="Networks currently active in this region..." />
        <Section label="v7 Regional Coverage (by state)" value={data?.v7_coverage ?? ''} editValue={editState.v7_coverage} editing={editing} onChange={(v) => setEditState((s) => ({ ...s, v7_coverage: v }))} placeholder="Coverage data will be added here..." />
        <Section label="v8 Regional Coverage (by state)" value={data?.v8_coverage ?? ''} editValue={editState.v8_coverage} editing={editing} onChange={(v) => setEditState((s) => ({ ...s, v8_coverage: v }))} placeholder="Coverage data will be added here..." />

        {/* Areas of Opportunity — from Supabase (synced from Notion) */}
        <div>
          <div className="mb-2 flex items-center justify-between">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-gray-500">Current Areas of Opportunity</h3>
            <a href={NOTION_DB_URL} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-blue-500 hover:text-blue-700">
          ...
          {opportunities ? (
              ...
              <div className="space-y-2">
                {opportunities.map((opp) => (
                  <a key={opp.id} href={opp.notion_url} target="_blank" rel="noopener noreferrer"
                    ...
                    {opp.region_num === 0 && (
                      <div className="mt-1 text-xs text-gray-400 italic">Cross-regional</div>
                    )}
                  </a>
                ))}
              </div>
            ...
            /* Fallback: render as plain text if not JSON format */
            <div className="rounded-lg bg-gray-50 px-4 py-3 text-sm text-gray-700 min-h-[60px]">
              {data?.areas_of_opportunity ? (
                <p className="whitespace-pre-wrap">{data.areas_of_opportunity}</p>
              ) : (
      ...
      {editing ? (
        <div className="border-t border-gray-100 px-5 py-4 flex gap-3">
          <button onClick={handleSave} disabled={saving} className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-[#001A41] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#002C5F] disabled:opacity-50 transition-colors">
            <Save className="h-4 w-4" />{saving ? 'Saving...' : 'Save Changes'}
          </button>
          <button onClick={handleCancel} disabled={saving} className="flex items-center justify-center gap-2 rounded-lg border border-gray-200 px-4 py-2.5 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">
...
function Section({ label, value, editValue, editing, onChange, placeholder }: {
  label: string; value: string; editValue: string; editing: boolean; onChange: (v: string) => void; placeholder: string;
}) {
  return (
    <div>
      <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-gray-500">{label}</h3>
      {editing ? (
        <textarea value={editValue} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} rows={4}
        ...
        <div className="rounded-lg bg-gray-50 px-4 py-3 text-sm text-gray-700 min-h-[60px]">
          {value ? <p className="whitespace-pre-wrap">{value}</p> : <p className="text-gray-300 italic">Not yet configured</p>}
        </div>
      )}
    </div>
  );
